<template>
  <div class="container mt-5">
    ...
    <form @submit.prevent="register">
      ...
      <div class="form-group">
        ...
        <input 
          ...
          required 
        />
        <div v-if="errors.license" class="invalid-feedback">
          {{ errors.license }}
        </div>
      </div>
      <button 
    ...
    <div class="mt-3">
      <router-link to="/pharmacist/login" class="btn btn-link">Already have an account? Login</router-link>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    register() {
      ...
      try {
        const user = { ...this.form, role: 'pharmacist' };
        this.$store.commit('auth/registerUser', user);
        
        // Show success message
        this.registerStatus = {
          type: 'success',
          message: 'Pharmacist registered successfully! Redirecting to login...'
        };
        
        // Redirect after a short delay
        setTimeout(() => {
          this.$router.push('/pharmacist/login');
        }, 1500);
      } catch (err) {
