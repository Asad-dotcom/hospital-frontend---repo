<template>
  <div class="container mt-5">
    ...
    <div class="mt-3">
      <router-link to="/receptionist/login" class="btn btn-link">Already have an account? Login</router-link>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    ...
    register() {
      ...
      try {
        const user = { ...this.form, role: 'receptionist' };
        this.$store.commit('auth/registerUser', user);
        
        // Show success message
        this.registerStatus = {
          type: 'success',
          message: 'Receptionist registered successfully! Redirecting to login...'
        };
